...
app.get('/campground/:id/update', (req, res) => {
    const { id } = req.params;
  ...
  res.redirect('update',{campground});
});

app.patch('/campground/:id/update', async(req, res) => {
    const { id } = req.params;
    const { title, price, description, location } = req.body;
    await Campground.findByIdAndUpdate(id,{ title, price, description, location })
    .then((d) => {
      console.log(d);
    })
    .catch((err) => {
